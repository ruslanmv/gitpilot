...
async def run_snippet(language: str, code: str, timeout: int) -> str:
    ...
    # GitPilot's own backend defaults to :8000 (``cli.DEFAULT_PORT``).
    # This used to fall back to :8765 — MatrixLab's host port — so with
    # no GITPILOT_PORT set the agent POSTed its snippets either at a
    # MatrixLab Runner that has no /api/sandbox/run (404) or at nothing
    # at all ("could not reach the in-process sandbox API").
    port = os.environ.get("GITPILOT_PORT") or "8000"
