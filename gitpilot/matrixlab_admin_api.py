...
class MatrixLabStatus(BaseModel):
    ...
    # When the persisted ``runnerUrl`` is unreachable but a different
    # candidate URL (e.g. localhost:8765 vs the stale localhost:8000)
    # responds on /health, the modal surfaces a one-click "Use
    # ``discoveredUrl``" CTA.  ``None`` when no discrepancy is found.
    discoveredUrl: Optional[str] = None
...
async def _probe_health() -> tuple[bool, Optional[str], Optional[str]]:
    ...
    return await _probe_url(base)


async def _probe_url(base: str) -> tuple[bool, Optional[str], Optional[str]]:
    """Health-probe one explicit URL.  Same shape as :func:`_probe_health`
    so the auto-detect path can reuse the parsing logic."""
    try:
...
# Candidate URLs the auto-detector tries when the persisted runner URL
# doesn't respond.  Kept short and deterministic so the modal's
# "looking for runner..." spinner doesn't blow past 1-2s.
#
# Order matters: the canonical post-:8000-collision port (8765) comes
# first because that's where ``make install`` puts the runner today.
# The stale :8000 default is included for completeness — when a user
# is mid-migration, this is the URL their persisted settings still
# point at.
_CANDIDATE_URLS = (
    "http://localhost:8765",
    "http://127.0.0.1:8765",
...
async def _discover_live_url(current_url: str) -> Optional[str]:
    """Return the first candidate URL whose /health responds, excluding
    ``current_url`` (which the caller has already established is down).
    None when nothing else answers — keeps the call honest about the
    "configured URL is stale" advisory.
    """
    current_norm = (current_url or "").rstrip("/")
    for candidate in _CANDIDATE_URLS:
        if candidate == current_norm:
            continue
        reachable, _err, _ver = await _probe_url(candidate)
        if reachable:
            return candidate
    return None
...
async def _current_status() -> MatrixLabStatus:
    ...
    # Stale-URL heal: when the configured runner URL is down but a
    # different candidate URL responds on /health, surface it as
    # ``discoveredUrl`` so the modal can show a one-click "Use ..." CTA.
    # Especially common after the legacy :8000 → :8765 port migration —
    # users have a runner up on 8765 but persisted settings still say
    # 8000.  Cheap (one or two ~30ms HTTP attempts), so we do it on
    # every status fetch rather than gating behind a refresh button.
    if not reachable:
        try:
            discovered = await _discover_live_url(get_settings().sandbox.matrixlab_url or "")
            if discovered:
                status = status.model_copy(update={"discoveredUrl": discovered})
        except Exception:  # pragma: no cover - never break status on a probe error
            pass
    return status
...
@router.post("/url/auto-detect", response_model=MatrixLabStatus)
async def matrixlab_url_auto_detect() -> MatrixLabStatus:
    """Probe candidate URLs and adopt the first one that responds.

    The "stale URL" recovery primitive — used directly by the modal's
    "Use ``discoveredUrl``" CTA, and by ``/repair`` before it touches
    the container.  Idempotent: when the configured URL is already
    live, this is a no-op and returns the current status unchanged.
    """
    reachable, _err, _ver = await _probe_health()
    if reachable:
        return await _current_status()
    current = get_settings().sandbox.matrixlab_url or ""
    discovered = await _discover_live_url(current)
    if discovered is None:
        status = await _current_status()
        return status.model_copy(update={
            "status": "needs_attention",
            "errorCode": "NO_RUNNER_FOUND",
            "message": (
                "No MatrixLab runner found on the candidate ports. "
                "Start one with: make install-matrixlab"
            ),
        })
    # Adopt the discovered URL via the canonical config writer so the
    # change persists and any in-process sandbox instance gets refreshed.
    try:
        await _legacy_config(SandboxConfigUpdate(matrixlab_url=discovered))
    except HTTPException as exc:
        status = await _current_status()
        return status.model_copy(update={
            "status": "failed",
            "errorCode": "CONFIG_UPDATE_FAILED",
            "message": f"Found runner at {discovered} but could not persist the URL.",
            "technicalDetails": TechnicalDetails(rawError=str(exc.detail)),
        })
    return await _current_status()
...
async def matrixlab_repair() -> MatrixLabStatus:
    ...
    Strategy:
      1. URL auto-detect first — most "needs_attention" cases on
         upgrading installs are stale persisted URLs from the
         :8000 → :8765 port move; switching the URL alone fixes them
         without touching the container.
      2. If still unreachable, restart the runner container (``docker
         stop`` + ``docker start``) and re-probe.
      3. Re-activate the MatrixLab backend on success.

    Idempotent — safe to call from any non-``ready`` state.  Won't
    pull images or re-create the container; for that use ``/reinstall``.
    """
    if not _lifecycle_enabled():
        status = await _current_status()
        return status.model_copy(update={
    ...
    # 0. Cheap win: try URL auto-detect before restarting anything.
    #    Saves a 5-10s restart cycle when the only thing wrong is a
    #    stale persisted URL.
    detected_status = await matrixlab_url_auto_detect()
    if detected_status.status == "ready":
        try:
            await _legacy_config(SandboxConfigUpdate(backend="matrixlab"))
            detected_status = detected_status.model_copy(update={"activeSandbox": "matrixlab"})
    ...
    # 3. Re-detect URL (the just-started container may bind a fresh
    #    port mapping) and re-activate if healthy.
    status = await matrixlab_url_auto_detect()
...
async def matrixlab_logs(tail: int = 200) -> Dict[str, Any]:
    ...
    distinctly, with an actionable ``error`` when docker isn't available
    or the container doesn't exist yet.  When the expected container
    is missing we also list any matrixlab-shaped containers we can
    find so the operator knows the runner is up under a different name.
    """
    ...
    if not _docker_available():
        return {
            "ok": False,
            "error": "Docker is not available on this GitPilot backend.",
            "errorCode": "DOCKER_NOT_INSTALLED",
            "hint": "Install Docker, then re-run: make install-matrixlab",
            "lines": [],
    ...
    if step.exit_code != 0:
        ...
        looks_like_missing = (
            ...
            or "no such object" in stderr.lower()
        )
        # When the expected container is absent, list any other
        # matrixlab-shaped containers so the user knows whether the
        # install ran but produced a different name (env override),
        # or simply never ran at all.
        candidates: List[str] = []
        if looks_like_missing:
            try:
                list_step = await _run_shell(
                    ["docker", "ps", "-a", "--format", "{{.Names}}\t{{.Status}}"],
                    timeout=10,
                )
                for raw in (list_step.stdout or "").splitlines():
                    name = raw.split("\t", 1)[0].strip()
                    if name and "matrixlab" in name.lower():
        ...
        if looks_like_missing:
            if candidates:
                friendly = (
                    f"No container named '{DEFAULT_CONTAINER_NAME}'. "
                    f"Found other matrixlab-shaped containers — set "
                    f"GITPILOT_MATRIXLAB_CONTAINER to one of them, or "
                    f"reinstall to recreate the expected one."
                )
            else:
                friendly = (
                    f"No container named '{DEFAULT_CONTAINER_NAME}' exists yet. "
                    f"Install with: make install-matrixlab"
                )
            return {
                "ok": False,
                "error": friendly,
                "errorCode": "CONTAINER_NOT_FOUND",
                "hint": "make install-matrixlab",
                "lines": [],
                "container": DEFAULT_CONTAINER_NAME,
        ...
        return {
            ...
            "lines": (stderr or step.stdout or "").splitlines(),
            "container": DEFAULT_CONTAINER_NAME,
            "rawError": stderr,
        }
