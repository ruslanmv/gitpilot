...
import type { ChatIntent, ChatMessagePayload, StructuredContextBundle, StructuredProjectContext, WorkspaceMode } from "../core/types";
...
export class ChatOrchestrator {
  ...
  async buildStructuredContext(input: {
    ...
    this.stateStore.updateProjectContextSummary({
      ...
      recentFiles: workingSet.recentFiles,
      readmeFound: Boolean(projectContext?.readmePreview),
  ...
  async sendMessage(input: {
    ...
    this.stateStore.appendChatMessage(userMessage);
    this.stateStore.updateUi({ mode: "working", notice: undefined });
    ...
    try {
      ...
      const responsePlan = response.plan || provisionalPlan;
      ...
      this.stateStore.appendChatMessage(assistantMessage);
      this.stateStore.updateActiveTask({
        title: input.rawText.slice(0, 80),
        intent: String(input.intent),
      ...
      const message = this.errorTranslator.translate(error);
      this.stateStore.appendChatMessage({
        id: `system:${Date.now()}`,
        role: "system",
