...
async def test_native_status_reports_not_installed_for_empty_dir(tmp_path, monkeypatch) -> None:
    """Fresh host, nothing cloned: native status reports installed=False.

    Native async test (pytest-asyncio's auto mode runs it on a clean
    event loop).  ``asyncio.get_event_loop().run_until_complete(...)``
    breaks when a previous async test mutated the global loop state —
    proper ``async def`` avoids that whole class of flakes.
    """
    monkeypatch.setattr(mla, "MATRIXLAB_LOCAL_DIR", tmp_path / "ml")
    monkeypatch.setattr(mla, "MATRIXLAB_PID_FILE", tmp_path / "ml" / ".pid")

    info = await mla._native_status()
...
async def test_native_status_detects_full_layout(tmp_path, monkeypatch) -> None:
